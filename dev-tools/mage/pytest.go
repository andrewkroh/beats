...
type PythonTestArgs struct {
	...
	VirtualenvReqs      string            // requirements.txt file
...
func PythonVirtualenv(requirementsTxt string) (string, error) {
	...
	// Only execute if requirements.txt is newer than the virtualenv dir.
	if IsUpToDate(ve, requirementsTxt) {
	...
	if err := sh.RunWith(env, virtualenvPath(ve, "pip"),
		"install", "--quiet", "--upgrade",
		"--requirement", requirementsTxt); err != nil {
		return "", err
	}
