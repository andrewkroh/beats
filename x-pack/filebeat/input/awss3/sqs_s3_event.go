...
func newSQSS3EventProcessor(log *logp.Logger, sqs sqsAPI, sqsVisibilityTimeout time.Duration, s3 s3ObjectHandler) *sqsS3EventProcessor {
...
func (p *sqsS3EventProcessor) ProcessSQS(ctx context.Context, msg *sqs.Message) error {
	...
	keepaliveWg.Add(1)
	go p.keepalive(ctx, log, &keepaliveWg, msg)

	err := p.processS3Events(ctx, log, *msg.Body)
	switch err.(type) {
	case nil:
		// No error. Delete SQS.
		return errors.Wrap(p.sqs.DeleteMessage(ctx, msg), "failed deleting message from SQS queue (it may be reprocessed)")
	case *nonRetryableError:
		// An error that reprocessing cannot correct. Delete SQS.
		return multierr.Combine(
			errors.Wrap(err, "failed processing SQS message (message will be deleted)"),
			errors.Wrap(p.sqs.DeleteMessage(ctx, msg), "failed deleting message from SQS queue (it may be reprocessed)"),
		)
	default:
		// An error that may be resolved by putting the message back on SQS
		...
		if msgVisErr := p.sqs.ChangeMessageVisibility(ctx, msg, 0); msgVisErr != nil {
			log.Debug("Failed to immediately return SQS message to queue by "+
				"setting visibility timeout to 0.", "error", msgVisErr)
		}
		return errors.Wrap(err, "failed processing SQS message (returning it to the queue for reprocessing)")
	}
}
