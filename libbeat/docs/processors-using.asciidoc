...
And this condition returns true when `destination.ip` is within either of the
given subnets.
...
Parameters can be passed to the script by adding `script_params` to the config.
This allows for a script to be made reusable. When using `script_params` the
...
`type`:: This field is required and its value must be `javascript`.

`id`:: This is an optional identifier that is added to log messages. If defined
...
`script_params`:: A dictionary of parameters that are passed to the `register`
of the script.
...
|`Get(string)`
|Get a value from the event. If the key does not exist `null` is
returned. If no key is provided then an object containing all fields is
returned.
